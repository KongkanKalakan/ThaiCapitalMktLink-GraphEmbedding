...
class HDMI(embedder):
    ...
    def training(self):
        ...
        path = './output/{}_dim{}.emb'
        
        df = pd.DataFrame(embeds.cpu().numpy()).reset_index()
        df['index'] = df['index']+1

        with open(path.format(self.args.dataset, self.args.hid_units), 'w') as f:
            f.write('14 {}\n'.format(self.args.hid_units))
            df.to_csv(f, index=False, header=False, sep='\t', line_terminator='\n')
        accs, macro_f1s, micro_f1s, k1, st = evaluate(embeds, self.idx_train, self.idx_val, self.idx_test, self.labels,)
        
        path = './output/{}_dim{}_result.csv'
        df_result = pd.DataFrame([[self.args.dataset, self.args.hid_units,
                            np.mean(accs), np.std(accs), 
                            np.mean(macro_f1s), np.std(macro_f1s), 
                            np.mean(micro_f1s), np.std(micro_f1s), 
                            np.mean(k1), np.std(k1), 
                            st[0], st[1]]], 
                            columns=['dataset', 'hid_units',
                            'mean_accu', 'std_accu', 'mean_macro_f1', 'std_macro_f1', 
                            'mean_micro_f1', 'std_micro_f1', 'mean_NMI', 'std_NMI', 'sim_5', 'sim_10'])
        df_result.to_csv(path.format(self.args.dataset, self.args.hid_units), index=False)
